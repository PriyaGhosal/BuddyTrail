...
Contributors
    try {
        const contributors = [];
        let page = 1;
        let totalContributions = 0; // Initialize total contributions
        let moreContributors = true;

        // Fetch contributors in a loop until no more pages
        while (moreContributors) {
            const response = await fetch(`https://api.github.com/repos/PriyaGhosal/BuddyTrail/contributors?per_page=100&page=${page}`);
            
            // Check for rate limiting
            if (response.status === 403) {
                console.warn('Rate limit exceeded. Waiting for reset...');
                const resetTime = response.headers.get('X-RateLimit-Reset') * 1000; // Convert to milliseconds
            ...
            if (data.length > 0) {
                ...
                totalContributions += data.reduce((sum, contributor) => sum + contributor.contributions, 0);
                page++;
            } else {
        ...
        return { contributors, repoStats: repoData, totalContributions }; // Return total contributions
    } catch (error) {
...
Contributors
    ...
    const stats = [
        ...
        { label: 'Total Contributions', value: totalContributions, icon: 'git-commit' }, // Use total contributions
...
Contributors
    ...
    const { contributors, repoStats, totalContributions } = await fetchData(); // Get total contributions

    renderStats(repoStats, contributors.length, totalContributions); // Pass total contributions
